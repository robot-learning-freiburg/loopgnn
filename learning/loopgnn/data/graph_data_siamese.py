...
class GraphDatasetSiamese(torch_geometric.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.scene_wise and not self.cross_seq_loops:
            ...
            topk = min(int(self.params.preprocessing.retrieval_ratio * len(self.kf_data.keyframes[scene_idx])), 100)
            ...
            topk = min(int(self.params.preprocessing.retrieval_ratio * len(self.kf_data.keyframes)), 100)
            scene_idx = self.kf_data.sample2scene[idx]
        ...
        if self.scene_wise:
            ...
            # the images themselves will constitute the node features
            node_feats = []
            for node_idx in neighborhood_ind:
                img = tfm.ToTensor()(Image.open(self.kf_data.left_image_files[scene_idx][node_idx]["left_image"]).convert("RGB"))
                if cut_below is not None:
                    img[:,cut_below:] = 0
                if resize is not None:
                    img = tfm.Resize(resize, antialias=True)(img)
                node_feats.append(img)
            node_feats = torch.stack(node_feats).to(torch.float32).reshape(-1, 512*512*3)  # Todo: whats the problem here                                                                            self.params.preprocessing.kp[self.kp_method].num_kp * self.params.preprocessing.kp[self.kp_method].desc_dim)
            ...
            node_feats = []
            for node_idx in neighborhood_ind:
                img = tfm.ToTensor()(Image.open(self.kf_data.left_image_files[node_idx]["left_image"]).convert("RGB"))
                if cut_below is not None:
                    img[:,cut_below:] = 0
                if resize is not None:
                    img = tfm.Resize(resize, antialias=True)(img)
