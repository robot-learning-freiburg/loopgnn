""" 
Adapted from AnyLoc (https://github.com/AnyLoc/AnyLoc/blob/a9fda68c55083f765b019df148a1b67614f90ee2/utilities.py#L624), 2025.
"""
import os
import random
...
def get_top_k_recall(top_k: List[int], db: torch.Tensor, 
    ...
    # Get the max(top-k) retrieval, tnen calculate remaining recalls
    index.add(db)
    distances, indices = index.search(qu, max(top_k))
    recalls = dict(zip(top_k, [0]*len(top_k)))
    # print(qu.shape,indices.shape)
    for k in top_k:
        recalls[k] = np.sum(gt_pos[indices[:,:k]]) / min(np.sum(gt_pos), k)
    return distances, indices, recalls

# def compute_cosine_sim(qu: torch.Tensor, db: torch.Tensor, norm_descs: bool=True):
#     if len(qu.shape) == 1:
#         qu = qu.unsqueeze(0)
#     if norm_descs:
#         db = F.normalize(db)
#         qu = F.normalize(qu)
#     dist = torch.nn.functional.cosine_similarity(qu, db)
#     return dist


def compute_cosine_sim(qu: torch.Tensor, db: torch.Tensor, method: str = "cosine", use_gpu: bool = False, norm_descs: bool = True):
    if len(qu.shape) == 1:
        qu = qu.unsqueeze(0)
    if norm_descs:
        db = F.normalize(db)
    ...
    # Get the max(top-k) retrieval, tnen calculate remaining recalls
    index.add(db)
    distances, indices = index.search(qu, db.shape[0])

    return distances, indices

def compute_retrieval_recall(dataset, eval_ratios: List[float] = [0.01, 0.02, 0.05, 0.10, 0.20]):
    """
    Given a dataset, computes the frame retrieval recall at different percentual thresholds.

    Parameters:
    - dataset:     The dataset object with keyframes and ground truth loop closures.
    - eval_ratios: The ratios at which the recall is computed. Eg: [0.01, 0.02, 0.05, 0.10, 0.20]

    Returns:
    - sum_recall:  The recall for each eval_ratio. The recall is the average of all keyframes.
    """
    assert dataset is not None, "Dataset is None -> please provide a TD2 keyframe dataset instance"
    # create recall dict from eval_threshs
    eval_threshs = [int(i*len(dataset.keyframes)) for i in eval_ratios]
    recall_overall = {eval_ratios[i]: [] for i in range(len(eval_ratios))}
    for i in tqdm.tqdm(range(len(dataset.keyframes)), total=len(dataset.keyframes), desc="Computing recall over all frames"):
        query_idx = i
        if query_idx % 20 == 0:
            dist, idcs, recall = get_top_k_recall(top_k=eval_threshs,
                                                db=dataset.global_desc,
                                                qu=dataset.global_desc[query_idx],
                                                gt_pos=dataset.gt_loop_matrix[query_idx], 
                                                method="cosine",
                                                use_gpu=False,
                                                )
            for rat, topk in zip(eval_ratios, eval_threshs):
                recall_overall[rat].append(float(recall[topk]))

    # sum list of recalls
    sum_recall = {rat: float(sum(recall_overall[rat])/len(recall_overall[rat])) for rat in eval_ratios}
    return sum_recall